...
class VideoUserParse(BaseParse):
    ...
    def videoParse(self, channel, url,userId):
        ...
        div = soup.first("div",{"class":"listbox board clearfix"})
        if div!=None:
            lis = div.findAll("li")
            if len(lis)==0:
                return False
            for li in lis:
                ...
                obj['url'] = mp4Url
                img = li.first("img")
                obj['pic'] = img.get("data-echo")
                obj['name'] = ahref.get("title")
    
                videourl = urlparse(obj['url'])
                obj['path'] = "1009tv_"+videourl.path
                obj['rate'] = 1.2
                obj['updateTime'] = datetime.datetime.now() 
                obj['userId'] = userId
                obj['baseUrl'] = baseurl
    ...
    def parseDomVideo(self, url):
        try:
            Id = url.replace("/infodetail/","").replace(".html","")
            url = "/playing/%s-1-1.html"%(Id)
            soup = self.fetchUrl(url)
