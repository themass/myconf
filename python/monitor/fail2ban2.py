...
reg = re.compile(
    r"banned ([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+) from ")
# Configuration
# -------------
data = []
# Email address
MAILADDR = "freevpn_account@163.com"
...
def parse_server(server, use_ssl, protocol):
    ...
    try:
        ...
        port = DEFAULT_PORT[protocol][
            use_ssl] if server_item_len == 1 else int(server_item[1])
    ...
    try:
        if not os.path.exists(result_path):
            ...
            if os.path.isfile(result_path):
                ...
                print(
                    "[*] Directory {0} has already existed".format(result_path))
    except BaseException as e:
    ...
    try:
        if os.path.isdir(result_path):
            if len(os.listdir(result_path)) == 0:
                ...
                print(
                    "[*] Remove directory {0} successfully".format(result_path))
            else:
                print(
                    "[*] Directory {0} is not empty, no need remove".format(result_path))
        else:
...
def protocol_manager(protocol, host, port, usr, pwd, use_ssl, filter=''):
    ...
    if hasattr(__main__, protocol):
        getattr(__main__, protocol)(host, port, usr, pwd, use_ssl, filter)
    else:
        exit_script("Wrong protocol: {0}".format(protocol))


def pop3(host, port, usr, pwd, use_ssl, filter=''):
    """Pop3 handler
    ...
    try:
        conn = poplib.POP3_SSL(
            host, port) if use_ssl else poplib.POP3(host, port)
        conn.user(usr)
        conn.pass_(pwd)
...
def imap4(host, port, usr, pwd, use_ssl, filter=''):
    """Imap4 handler
    ...
    try:
        conn = imaplib.IMAP4_SSL(
            host, port) if use_ssl else imaplib.IMAP4(host, port)
        result, message = conn.login(usr, pwd)
    ...
    # Get all folders
    try:
        result, message = conn.select("INBOX.fail2ban")
        print result, message
    ...
    for folder in folders:
        if filter != '' and folder.count(filter) > 0:
            # Parse folder info and get folder name
            try:
                flags, delimiter, folder_name = list_pattern.match(
                    folder).groups()
...
#             else:  # Main content
#                 result_file = os.path.join(
#                     result_path, "mail{0}_text".format(i))
#
#             try:
...
if __name__ == "__main__":
    ...
    result_path = os.path.join(mailaddr)
    protocol = parse_protocol(protocol)
    host, port = parse_server(server, use_ssl, protocol)

#     create_dir(result_path)
    protocol_manager(
        protocol, host, port, mailaddr, password, use_ssl, 'fail2ban')
#     remove_dir(result_path)
    ...
    print(
        "[*] Finish download email of {0} in {1:.2f}s".format(mailaddr, exec_time))
