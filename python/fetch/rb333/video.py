...
class VideoUserParse(BaseParse):
    ...
    def run(self):
        ...
        print '333rb user video -- channel ok;,len=',len(chs)
        dbVPN.commit()
        dbVPN.close()
        for item in chs:
    ...
    def videoParse(self, channel, url,userId):
        ...
        div = soup.first("div", {"class": "col-lg-9"})
        if div!=None:
            lis = div.findAll("div",{"class":"col-md-3 portfolio-item new-video"})
            for li in lis:
    ...
    def parseDomVideo(self, url):
        try:
            ...
            adiv = soup.first("a",{"class":"play"})
            if adiv!=None:
                soup = self.fetchUrl(adiv.get("href"), header)
                div = soup.first("div",{'class':'col-md-12 center'})
                if div!=None:
