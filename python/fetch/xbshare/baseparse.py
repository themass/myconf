...
maxCount = 3
regVideo = re.compile(r".siteLink = \"http(.*?)m3u8\";")
    ...
    def fetchUrl(self, url, aheader=header):
        ...
        while count < maxCount:
            try:
                ...
                soup = BeautifulSoup(content.replace("<!--[if lt IE 9]>", "").replace("<![endif]-->", ""))
                return soup
...
#         content = self.fetchContentUrl(headerUrl, header)
        ...
        with open("%s/%s"%("xbshare",name)) as f:
        ...
        while count < maxCount:
            try:
                ...
                if gzipped:
                    content = zlib.decompress(
                        content, -zlib.MAX_WBITS)  # 解压缩，得到网页源码
                return content.replace("<!--[if lt IE 9]>", "").replace("<![endif]-->", "")
            except Exception as e:
                print common.format_exception(e)
                print '打开页面错误,重试', baseurl+url, '次数', count
                count = count + 1
