    ...
    def run(self):
        ...
        for item in chs:
            ...
            for i in range(20, maxVideoPage):
                con = self.videoParse(item['channel'], item['channelType'],'%s%s%s'%(url,'?page=',i))
                if con==False:
...
#         channelList.reverse()
    ...
    def parseDomVideo(self, url):
        try:
            soup = self.fetchUrl(url)
            iframe = soup.first("iframe",{'id':'videoIframe'})
            if iframe != None:
                mp4UrlSoup = self.fetchUrlWithBase(iframe.get("src"))
                if mp4UrlSoup != None:
                    source = mp4UrlSoup.first("source")
                    if source != None:
